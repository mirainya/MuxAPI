...
func (s *Server) recordRequest(requestID string, started time.Time, groupID int64, keyName, model, endpoint, clientIP, userAgent string, stream bool, requestBytes int64, result forward.Result) {
	...
	for i, attempt := range result.Attempts {
		attempts[i] = store.RequestAttemptRecord{
			...
			LastEvent: attempt.LastEvent, InputTokens: attempt.InputTokens, InputTokensNormalized: attempt.InputTokensNormalized, OutputTokens: attempt.OutputTokens,
	...
	s.store.EnqueueRequest(store.RequestRecord{
		...
		ResponseBytes: result.ResponseBytes, InputTokens: result.InputTokens, InputTokensNormalized: result.InputTokensNormalized, OutputTokens: result.OutputTokens,
